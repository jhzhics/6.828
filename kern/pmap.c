...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		...
		pages_left = npages - (int32_t)nextfree / PGSIZE;
	}
	...
	result = nextfree;
	int pages_alloc = (n + PGSIZE - 1) / PGSIZE;
	nextfree += pages_alloc * PGSIZE;	// also works for n = 0

	pages_left -= pages_alloc;
	if (pages_left < 0)
	{
...
mem_init_mp(void)
{
	...
	{
		boot_map_region(kern_pgdir, KSTACKTOP - (i + 1) * (KSTKSIZE + KSTKGAP) + KSTKGAP, KSTKSIZE,
...
page_init(void)
{
	...
	uint32_t free_mem_idx[2][2];
	{
		char *end = boot_alloc(0);
		free_mem_idx[0][0] = 1;
		free_mem_idx[0][1] = npages_basemem;
		free_mem_idx[1][0] = ((uint32_t)end -KERNBASE + PGSIZE - 1) / PGSIZE;
		free_mem_idx[1][1] = npages;
	}

	int MPENTRY_PGIDX = PGNUM(MPENTRY_PADDR);

	// cprintf("%x\n%x\n", free_mem_idx[1][0] * PGSIZE, EXTPHYSMEM);
	
	uint32_t intervals_n = sizeof(free_mem_idx) / sizeof(free_mem_idx[0]);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (const void *i = begin; i < end; i += PGSIZE)
	{
		...
		if (!pte || (~*pte & (perm | PTE_P)) || (uintptr_t)i >= ULIM)
		{
