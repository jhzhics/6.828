...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (void *va_cur = va_begin; va_cur < va_end; va_cur += PGSIZE) {
		...
		if (page_insert(e->env_pgdir, pp, va_cur, PTE_U | PTE_W) < 0) {
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	if (elf->e_magic != ELF_MAGIC) {
		panic("load_icode: invalid ELF magic");
	}
	...
	for (; ph < eph; ph++) {
		...
		lcr3(PADDR(e->env_pgdir));	// So ph->p_va is valid
		memcpy((void *) ph->p_va, binary + ph->p_offset, ph->p_filesz);
		lcr3(PADDR(kern_pgdir)); // Restore kern_pgdir
	}
	// Set the entry point for the new environment
